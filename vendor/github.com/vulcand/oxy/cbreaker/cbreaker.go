...
// NewRT creates a new CircuitBreaker middleware
func NewRT(expression string, options ...CircuitBreakerOption) (*CircuitBreaker, error) {
	cb := &CircuitBreaker{
		m:      &sync.RWMutex{},
...
func (c *CircuitBreaker) RoundTripWithDelegate(req *http.Request, delegate http.RoundTripper) (*http.Response, error) {
	if c.activateFallback() {
		return c.fallbackRT.RoundTrip(req)
	}
	return c.serveRT(req, delegate)
}
...
func (c *CircuitBreaker) activateFallback() bool {
...
func (c *CircuitBreaker) serveRT(req *http.Request, delegate http.RoundTripper) (*http.Response, error) {
	...
	var statusCode int
	if err != nil {
		// report any error as a server error
...
var defaultFallbackRT = &fallbackRT{}

var SentinelErr = errors.New("blah")

type fallbackRT struct{}

func (frt *fallbackRT) RoundTrip(_ *http.Request) (*http.Response, error) {
